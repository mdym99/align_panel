...
def align_auto(ref_image, mov_image, align_type:str, inverse = True, sub_pixel_factor = 2):
    ...
    if align_type == "PyStackReg_translation":
        ...
        shifts = estimate_image_shift(ref_image, mov_image,sub_pixel_factor=sub_pixel_factor)
        shifts = np.flip(shifts[0])
        trans.translate(xshift=shifts[0], yshift=shifts[1])
    elif align_type == "cross_corelation_skimage":
        shifts, error, phasediff = phase_cross_correlation(ref_image, mov_image, upsample_factor=sub_pixel_factor)
        trans.translate(xshift=shifts[0], yshift=shifts[1])
...
def crop_images(ref_image, mov_image, rebin = 8):
    ...
    fig.canvas.mpl_connect('key_press_event', partial(toggle_selector, selectors,fig ,centers, positions))
    fig.canvas.draw()
